...
export class GooglemapsComponent implements OnInit {
  ...
  async addMarkers() {
    ...
    /* Centro de acopios */
     if(this.acopios){
      for (let i = 0; i < this.acopios.length; i++) {
        const marker = new google.maps.Marker({
          position: {
            lat: Number(this.acopios[i].position.lat),
            lng: Number(this.acopios[i].position.lng),
          },
          label: 'C.A',
          map: this.map,
        });
        marker.addListener('click', async () => {
          const modal = await this.modalCtrl.create({
            ...
            componentProps: {
              description: this.acopios[i].description,
              days: this.acopios[i].days,
              hours: this.acopios[i].hours,
              name: this.acopios[i].name,
              user: this.acopios[i].usuario,
              phone: this.acopios[i].phone,
            },
          });
          await modal.present();
        });
          ...
          marker.addListener('click', async () => {
            const modal = await this.modalCtrl.create({
              ...
              componentProps: {
                ...
                name: this.feature[i].name,
                user: this.feature[i].usuario,
                phone: this.feature[i].phone,
              },
            });
            await modal.present();
          });
